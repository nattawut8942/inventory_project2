                            ...
                            <div key={p.ProductID} className={`group bg-white border rounded-2xl p-5 transition-all hover:shadow-lg shadow-sm ${isLow ? 'border-red-100 ring-2 ring-red-50' : 'border-slate-200 hover:border-indigo-200'}`}>
                                <div className="flex justify-between items-start mb-3">
                                    ...
                                    <div className="flex gap-1">
                                        ...
                                        {isAdmin && (
                                            <>
                                                <button onClick={() => setEditItem(p)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg">
                                                    <Edit2 size={14} />
                                                </button>
                                                <button onClick={() => setShowDeleteConfirm(p.ProductID)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg">
                                                    <Trash2 size={14} />
                                                </button>
                                            </>
                                        )}
                                ...
                                <div className="flex justify-between items-end mt-4 pt-4 border-t border-slate-50">
                                    <div>
                                        ...
                                        <p className="text-slate-600 font-mono text-sm">฿{p.LastPrice?.toLocaleString()}</p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-[10px] text-slate-400 uppercase font-bold">Stock</p>
                                        <p className={`font-bold text-2xl font-mono ${isLow ? 'text-red-500' : 'text-emerald-600'}`}>{p.CurrentStock}</p>
                                    </div>
                                </div>
                            </div>
                        );
