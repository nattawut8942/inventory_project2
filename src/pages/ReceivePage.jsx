                    ...
                    <div className="bg-white w-full max-w-lg rounded-3xl border border-slate-200 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                        <div className="p-6 bg-slate-50 flex justify-between items-center border-b border-slate-200">
                            <div>
                                ...
                                <p className="text-xs text-slate-500 font-mono">PO: {activePo.PO_ID}</p>
                            </div>
                            <button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-slate-200 rounded-full text-slate-400 hover:text-slate-600">
                                <LogOut size={16} />
                            </button>
                        </div>
                        <form onSubmit={(e) => {
                            ...
                            const items = activePo.Items.map(it => ({
                                ...
                                Qty: Number(fd.get(`r-${it.DetailID}`)) || 0 // Explicit Number conversion
                            })).filter(it => it.Qty > 0);
                            ...
                            <div>
                                ...
                                <input
                                    name="inv"
                                    required
                                    className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl mt-1 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-slate-800"
                                    placeholder="IV-XXXXXXXX"
                                />
                            </div>
                            <div className="space-y-3">
                                <label className="text-[10px] font-bold text-slate-400 uppercase">Confirm Quantities</label>
                                {activePo.Items.map(item => {
                                    ...
                                    return (
                                        <div key={item.DetailID} className="flex items-center gap-4 bg-slate-50 p-3 rounded-xl border border-slate-100">
                                            <div className="flex-1 text-xs">
                                                <p className="font-bold text-slate-700">{prodName}</p>
                                                <p className="text-slate-400 text-[10px] mt-0.5">Ordering: {item.QtyOrdered} | Received: {item.QtyReceived}</p>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                ...
                                                <input
                                                    ...
                                                    max={rem}
                                                    defaultValue={rem}
                                                    className="w-16 bg-white border border-slate-300 rounded-lg p-2 text-center text-sm font-bold outline-none focus:border-indigo-500 text-slate-800"
                                                />
                                            </div>
                                        </div>
                                    );
